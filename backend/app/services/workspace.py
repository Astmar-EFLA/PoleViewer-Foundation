...
def resolve_workspace_path(relative_path: str) -> Path:
    """
    Resolves a workspace-relative path requested by the frontend, rejecting
    absolute paths, `..` traversal, and any result that would land outside
    the workspace root (including via a symlink) -- a wrong or malicious
    `filePath` in a request must fail loudly, not silently read (or write)
    ...
    if candidate.is_absolute():
        raise WorkspacePathError(f"filePath must be workspace-relative, got an absolute path: {relative_path!r}")
